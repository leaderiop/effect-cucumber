...
[BEH-EC-007](behaviors/02-shared-layers-and-tags.md)). All of it stress-tested against
...
3. ~~Finish `@effect-cucumber/vitest`'s register→plan→emit pipeline.~~ **Done in
   ...
   **plan** (`Plan.ts` resolves every Pickle step against the registered
   ...
   `ScenarioEffect.ts` are unchanged in behaviour on both paths. What remains on
   this package is Phase 11's dogfooded acceptance suite and the doc-examples
   compile check, both already listed in this file.
