...
DocString wrapper have shipped; `@effect-cucumber/vitest` has its registration
surface but no runner.** `loadFeature`/`parseFeature` and the `ParsedFeature`
...
every push by `scripts/verify-tsgo-gate.sh`. What is still an intended contract
only: the runner itself (nothing emits `it.effect` yet), plus hooks, tags,
Rules, Scenario Outlines and Layer scoping — stress-tested against
...
| Unit tests                                        | Yes for both. `packages/gherkin` has fourteen `test/*.test.ts` files over a `.feature` fixture set, plus one type-check-only `.types.ts` file compiled by `pnpm typecheck:test`. `packages/vitest` has three — `Registry.test.ts`, `Step.test.ts`, `describeFeature.test.ts` — all mapped in [`spec/traceability.md`](traceability.md) §4, plus eight compile-gate fixtures under `test/tsgo-gate/src/` that vitest never collects (most are deliberately non-compiling) and that `scripts/verify-tsgo-gate.sh` asserts in nine checks on every push                                                                                                                                                                                                                                                                                                                   |
...
3. Finish `@effect-cucumber/vitest`'s register→plan→emit pipeline. The
   **register** stage is done (BEH-EC-002–004: `describeFeature`, both Layer
   forms, the dsl containers, the step auto-wrap, and the compile gate that
   makes an unprovided service a type error). **Plan and emit** are not: nothing
   turns a collected Feature into `it.effect` calls yet, and step drift
   (BEH-EC-013) is unimplemented. Do that against one hand-written `.feature`
   file, proving out Background + Scenario + one Given/When/Then before
