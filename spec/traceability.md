...
`Step.ts`, `Registry.ts` and the `index.ts` barrel — and `packages/vitest/test`
has real suites. Everything else this document names under `packages/vitest`
...
| -------------------------------------------------------------------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| [01 — Steps and World](behaviors/01-steps-and-world.md)                                      | BEH-EC-001–004, BEH-EC-013 | `packages/gherkin/src/loadFeature.ts`, `packages/vitest/src/{describeFeature,Dsl,Step,Registry}.ts`, `packages/vitest/src/Plan.ts` |
...
| `packages/vitest/test/Registry.test.ts`                | BEH-EC-002, BEH-EC-003 | Per-instance step registration: two registries in one process share no state, `definitions()` is a snapshot, the scope stack refuses to underflow, and a definition carries the scope current when it was registered                       |
...
| `packages/vitest/test/describeFeature.test.ts`         | BEH-EC-002, BEH-EC-004 | Collection through `collectFeature`: one registry per call, the container a step was registered inside (ADR-EC-017), the define callback's synchrony, and both Layer forms normalising to one Layer with `perScenario` winning a collision |
