...
`Background.ts`.
`Plan.ts`, `ScenarioEffect.ts` and `Runner.ts` are real but none of the three is
reachable from any user-facing call yet: `describeFeature` still discards its
collection rather than planning and emitting it, so nothing calls `planFeature`,
`buildScenarioEffect` or `emitFeature` outside their own test suites.
...
| -------------------------------------------------------------------------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| [01 — Steps and World](behaviors/01-steps-and-world.md)                                      | BEH-EC-001–004, BEH-EC-013 | `packages/gherkin/src/{loadFeature,Snippet}.ts`, `packages/vitest/src/{describeFeature,Dsl,Step,Registry,CallSite,Errors,Plan,TestApi,Runner}.ts` |
...
| [INV-EC-002](invariants.md#inv-ec-002-a-per-scenario-layer-is-fresh-every-scenario)                    | Per-Scenario Layer is fresh   | The Layer supplied once per Scenario Effect, rebuilt on every execution, with `Runner.ts` emitting one such Effect per Scenario | `packages/vitest/test/ScenarioEffect.test.ts` — per-execution freshness only; isolation ACROSS two Scenarios stays untested until the pipeline is wired end to end |
