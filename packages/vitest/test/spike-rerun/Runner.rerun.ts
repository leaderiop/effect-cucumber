/**
 * SPIKE (issue #34) COPY of `packages/vitest/src/Runner.ts`, with the rerun-manifest filter added —
 * `main`'s real `Runner.ts` is untouched (`git diff main -- packages/vitest/src/Runner.ts` is
 * empty), same convention `packages/vitest/test/spike-attachments/` used for issue #33. Diff
 * against the real file with:
 *   diff packages/vitest/src/Runner.ts packages/vitest/test/spike-rerun/Runner.rerun.ts
 * every "SPIKE (issue #34)"-tagged line is the addition; everything else is copied verbatim,
 * imports retargeted to `../../src/...` since this file now lives one level deeper.
 *
...
import { emptyHookSet, type HookSet, mergeHookSets, runHookBatch } from "../../src/Hook.ts"
import { buildScenarioTitles } from "../../src/OutlineTitle.ts"
import type { ErasedExtraLayer, FeaturePlan, ScenarioPlan } from "../../src/Plan.ts"
import { buildScenarioEffect } from "../../src/ScenarioEffect.ts"
import { scenarioKey } from "../../src/ScenarioKey.ts"
import { isSkipped, shouldEmit, type TagFilter } from "../../src/Tags.ts"
import type { EmitOptions, TestApi } from "../../src/TestApi.ts"
// SPIKE (issue #34): the rerun-manifest key, computed once per `emitFeature` call alongside
// `titles` — see `RerunKey.ts` for why it cannot reuse `scenarioKey` above.
import { rerunKeysForPlan } from "./RerunKey.ts"

export interface EmitOutcome {
  readonly excludedScenarioCount: number
  /** SPIKE (issue #34): Scenarios excluded by `rerunFilter`, counted separately from tag-based
   * exclusions so `describeFeature.rerun.ts`'s existing includeTags/excludeTags notice doesn't fire
...
export const emitFeature = (
  args: {
    ...
    // SPIKE (issue #34): `null` means "no rerun filter" — every Scenario that survives `tagFilter`
    // is emitted, same as today. A non-null `Set` is the manifest's failed-key set; a Scenario whose
    // `RerunKey.ts` key is not in it is excluded, same shape as the tag filter above.
    readonly rerunFilter?: ReadonlySet<string> | null
    readonly onEmitted?: ((outcome: EmitOutcome) => void) | undefined
  }
): EmitOutcome => {
  const { api, hooks, layer, onEmitted, plan, rerunFilter = null, ruleHooks, ruleLayers, scenarioLayers, tagFilter } =
    args
  ...
  // SPIKE (issue #34): computed once, alongside `titles` below, only when a filter is active — a
  // Feature with no `rerunFailedOnly` option pays nothing extra.
  const rerunKeys = rerunFilter === null ? null : rerunKeysForPlan(plan)
  ...
  // SPIKE (issue #34): `true` when no filter is active, same convention as `shouldEmit`/`noTagFilter`.
  ...
  api.describe(plan.feature.name, () => {
    ...
    for (const scenario of plan.feature.scenarios) {
      ...
      // SPIKE (issue #34): checked AFTER the tag filter and counted separately, so a Scenario
      // excluded by both is attributed to tags (the pre-existing, better-understood mechanism).
    ...
    for (const rule of plan.feature.rules) {
      ...
      api.describe(rule.name, () => {
        for (const scenario of rule.scenarios) {
          ...
          // SPIKE (issue #34): same as the Feature-level loop above, written out for the reason
          // given there.
          if (!passesRerunFilter(scenarioPlan)) {
            rerunExcludedScenarioCount += 1
            continue
