 ...
 * Tests for `Plan`.
 *
 * Carries: ADR-EC-019, ADR-EC-022, ADR-EC-023, BEH-EC-016.
 */
import {
  type DataTable,
  type DocString,
...
// THREE steps that each carry a `.feature`-level argument, and the first two also carry a cucumber-expression
// parameter.
...
// Two Rules and a Feature-level Scenario, all THREE Scenarios named `shared` and all three worded identically.
const rules = parse(
  `Feature: Rules
...
// A tag at all FOUR levels — Feature, Rule, Scenario Outline and Examples block — over an Outline with TWO rows.
const tagged = parse(
  `@featuretag
Feature: Tagged
...
// `ruleId` defaults to `null` on all three, because a Feature-level frame genuinely is not nested in a Rule —
// Registry.ts note (e) reserves `null` for exactly that and for nothing else.
...
describe("planFeature — the step-argument join (BEH-EC-016)", () => {
  it("APPENDS a DataTable after the pattern's own coerced arguments", () => {
    ...
    // Length FIRST and by itself, so a regression that drops the tail is attributed to the drop rather than reported
    ...
    // The table is the STEP's, not some other step's — asserted through the accessor a body would actually call, so a
    // delivery that forwarded a correctly-tagged but wrong table is caught.
  ...
  it("APPENDS a DocString after the pattern's own coerced arguments, mediaType included", () => {
    ...
    // An `Option`, not a bare string — ADR-EC-022 put this whole surface on `Option<T>`, and an assertion that read
    // `"text/plain"` here would pass against a regression that unwrapped it.
...
describe("planFeature — the Rule level of the scope chain", () => {
  ...
  it("parses two distinct Rule ids for two Rules, so the fixture can discriminate at all", () => {
    // Not a tautology: every isolation assertion below is `===` between two ids, and all of them would pass vacuously
    // against a parse that minted one id for both Rules.
  ...
  it("lets a Rule-scope pattern resolve steps inside that Rule and nowhere else", () => {
    ...
    // The other Rule's Scenario is worded IDENTICALLY and named IDENTICALLY.
  ...
  it("never lets one Rule's registration serve another Rule's Scenario, even under one pattern text", () => {
    ...
    // NOT an ambiguity, even though both carry the same pattern at the same rank: neither was ever VISIBLE to the
    // other Rule's Scenario, so each Scenario saw exactly one match.
  ...
  it("does not let a Scenario-scope pattern cross into a same-named Scenario in a different Rule", () => {
    const plan = planFeature({
      feature: rules,
      // All three Scenarios in this fixture have astName `shared`, so `astName` alone selects all three.
...
describe("planFeature — three-level precedence, Scenario over Rule over Feature", () => {
  ...
  it("picks the Rule-scope match over the Feature-scope one when the Scenario level is absent", () => {
    // The load-bearing half of the three-level rank.
  ...
  it("does not report the shadowed Rule- and Feature-level patterns as unused", () => {
    // Both matched; both then lost to an inner level.
...
describe("planFeature — a Rule's own Background", () => {
  ...
  it("gives the Rule's Scenario a feature-background, a rule-background and a scenario step", () => {
    ...
    // The Feature-level Scenario has no rule-background step of its own, so it is served entirely by the
    // Feature-level registrations — nothing regressed for the pre-Phase-8 arrangement.
  ...
  it("no longer lets a Feature-level Background registration resolve a rule-background step", () => {
    const plan = planFeature({
      feature: ruleBackgrounds,
      // Registered in the FEATURE's Background (`ruleId: null`) but worded to match the RULE's Background step.
  ...
  it("serves a rule-background step from a Rule-scope registration, the Rule-level default", () => {
    const plan = planFeature({
      feature: ruleBackgrounds,
      // `rule` scope is the Rule-level analogue of `feature` scope: visible to EVERY step of every Scenario in that
      // Rule, Background steps included.
...
describe("planFeature — the Scenario's tag set", () => {
  ...
  it("gives every Examples row of an Outline the same tag set, the Examples-block tag included", () => {
    ...
    // Two rows, so "every row carries it" is discriminated from "the first row carries it".
    expect(plan.scenarios.map((scenario) => scenario.name)).toEqual(["adding 1", "adding 2"])
  ...
  it("plans an untagged Scenario with a present empty array, never a missing field", () => {
    // `single` is an untagged Scenario in an untagged Feature, so there is genuinely no tag anywhere to inherit.
...
describe("planFeature — , the undefined step", () => {
  ...
  it("names the step, its Feature and its line, with an empty matchedPatterns list", () => {
    ...
    // Hard-coded: line 6 of the `checkout` source above.
  ...
  it("carries a suggested snippet whose first line is the step's own registrar keyword", () => {
    ...
    // The step is written `When`, so the suggestion must be a `When` registrar — not the `Given` a keywordType-first
    // derivation would produce for a step whose type is `Action`.
    expect(suggestion?.split("\n")[0]?.startsWith("When(")).toBe(true)
    // Generalised, not literal: `3` became `{int}`.
  ...
  it("embeds the generated snippet in the message VERBATIM, never summarised", () => {
    const error = errorOf(nothingRegistered().scenarios[0]?.steps[1])
    // Generated independently here, against the same registry, so the assertion pins the exact bytes rather than a
    // shape this test also decides.
  ...
  it("derives the registrar keyword from keywordType when the literal keyword is not one", () => {
    ...
    // `*` is a legal Gherkin keyword and is no registrar's name, so the suggestion has to come from somewhere.
    expect(suggestion?.split("\n")[0]).toMatch(/^(Given|When|Then|And|But)\(/)
  })
})

describe("planFeature — , the ambiguous step", () => {
  ...
  it("orders line 9 before line 10 in the same file, numerically and not lexicographically", () => {
    // A comparison over the formatted `file:line:column` strings puts `:10:` before `:9:`, so this is the one
    // assertion in the repo that tells a numeric site order from a plausible string one.
...
describe("planFeature — , the unused step definition", () => {
  ...
  it("reports a Scenario-scope pattern whose text only ever appears in a different Scenario", () => {
    const plan = planFeature({
      feature: split,
      // Registered inside Scenario A, but `only in B` is a step of Scenario B — so this pattern was never once
      // VISIBLE to a step it could have matched.
  ...
  it("does NOT report a pattern that matched but lost to an inner-scope registration", () => {
    const plan = planFeature({
      feature: single,
      definitions: [
        // Matches the only step in the Feature, and is then shadowed by the Scenario-scope override below.
  ...
  it("carries the pattern, keyword, Feature name, uri and site, and says what to do", () => {
    ...
    // No line number in the prefix: the Feature has no single line for this finding, so the definition site carries
    // the location instead and appears in the sentences.
