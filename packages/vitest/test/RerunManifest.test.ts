 ...
 * `Option.none()` ("no filter") for three distinct reasons — only two of which warn — and
...
describe("readRerunManifest", () => {
  it("returns Option.none() with NO warning when the file does not exist yet — the ordinary first-run case", () => {
  ...
  it("returns Option.none() and warns once when the file exists but is not valid JSON", () => {
  ...
  it("returns Option.none() and warns once for `{}` — valid JSON, wrong shape (no `failed` field)", () => {
    const dir = mkdtempSync(join(tmpdir(), "effect-cucumber-rerun-manifest-"))
    const path = join(dir, "manifest.json")
    try {
  ...
  it("returns Option.none() and warns once for `{\"failed\": \"not an array\"}` — wrong shape (a string, not an array)", () => {
    const dir = mkdtempSync(join(tmpdir(), "effect-cucumber-rerun-manifest-"))
    const path = join(dir, "manifest.json")
    try {
  ...
  it("returns Option.none() and warns once for `{\"failed\": [1, 2, 3]}` — an array of the wrong element type", () => {
    const dir = mkdtempSync(join(tmpdir(), "effect-cucumber-rerun-manifest-"))
    const path = join(dir, "manifest.json")
    try {
  ...
  it("returns the exact expected Set wrapped in Option.some, with NO warning, for a real valid manifest file", () => {
    const dir = mkdtempSync(join(tmpdir(), "effect-cucumber-rerun-manifest-"))
    const path = join(dir, "manifest.json")
    try {
      ...
      const { result, warnCalls } = recordWarnings(() => readRerunManifest(path))
      assert.deepStrictEqual(result, Option.some(new Set(failed)))
      assert.strictEqual(warnCalls.length, 0)
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  ...
  it("returns an empty Set wrapped in Option.some, with no warning, for a valid manifest whose `failed` array is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "effect-cucumber-rerun-manifest-"))
    const path = join(dir, "manifest.json")
    try {
      writeFileSync(path, JSON.stringify({ failed: [] }), "utf8")
      const { result, warnCalls } = recordWarnings(() => readRerunManifest(path))
      assert.deepStrictEqual(result, Option.some(new Set()))
      assert.strictEqual(warnCalls.length, 0)
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
