...
- 4d206c7: Bumps `effect`/`@effect/platform-node`/`@effect/vitest` from `4.0.0-rc.115` to `4.0.0-rc.116`,
...
- 305b148: Un-vendors `@effect/vitest`, now that its `4.0.0-rc.113` release supports vitest 5 directly
...
- @effect-cucumber/gherkin@0.9.0

## 0.8.0

### Minor Changes

- d183081: Adds `GherkinJUnitReporter`, an optional vitest `Reporter` that writes JUnit XML with Scenario tags
...
- @effect-cucumber/gherkin@0.8.0

## 0.7.0

### Minor Changes

- dee5b1b: Supports vitest 5. The `vitest` peer range moves from `>=4.1.0 <5.0.0` to `>=5.0.0 <6.0.0` —
...
- @effect-cucumber/gherkin@0.7.0

## 0.6.0

### Minor Changes

- 1ed970f: `describeFeature` gains a `retry?: Schedule.Schedule<any, any, never>` registration option,
...
- 8467519: `Given`/`When`/`Then` gain a second call signature: a `decode` argument between the pattern and the
...
- Updated dependencies [1d32668]
  - @effect-cucumber/gherkin@0.5.0
...
- 812cd77: Add `scripts/templates/oxlint-ref-state/` (LINT-01): a copyable, real oxlint `jsPlugins` rule
