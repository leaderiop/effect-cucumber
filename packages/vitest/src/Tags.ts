/**
 * Reserved tags (`@skip`, `@only`) and the registration-time filter. `undefined` and `[]` both mean
 * no filter; where a tag is in both lists, exclude wins (BEH-EC-008, `test/Tags.test.ts`).
 */
