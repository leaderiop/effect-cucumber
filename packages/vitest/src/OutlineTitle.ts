...
export const buildScenarioTitles = (feature: ParsedFeature): ReadonlyMap<string, string> => {
  ...
  for (const scenario of feature.allScenarios) {
    // `onNone` here is a plain Scenario, whose title carries no row suffix at all.
    const base = Option.match(scenario.exampleRow, {
      onNone: () => scenario.name,
      ...
      onSome: (row) =>
        `${scenario.name} (${row.header.map((name, index) => `${name}=${row.values[index] ?? ""}`).join(", ")})`
    })
